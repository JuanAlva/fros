...
def generate_launch_description():
    ...
    tp_share = os.path.join(pkg_share, 'tp_final')
    ...
    robot_description = {
        'robot_description': ParameterValue(
            Command([
                ...
                os.path.join(tp_share, 'robot_description', 'mycobot_320_m5_2022', 'mycobot_320_m5_2022.xacro'),
                ' controllers_file:=',
                os.path.join(tp_share, 'config', 'mycobot_320_m5_2022', 'ros2_controllers.yaml'),
            ]),
            value_type=str
        )
    }
    ...
    # POSE INICIAL: con interfaz de esfuerzo, el controlador no sostiene ninguna
    # posicion hasta recibir un primer punto de trayectoria (si no, el brazo cae).
    exec_home_trajectory = ExecuteProcess(
        cmd=[
            'ros2', 'topic', 'pub', '--once',
    ...
    event_publish_home_trajectory = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=joint_trajectory_controller_spawner,
            on_exit=[
                TimerAction(period=1.0, actions=[exec_home_trajectory])
            ]
        )
    )

    # DESACTIVADO: con la garra sin mimic real, activarla vuelve inestable la simu.
    ...
    # event_launch_gripper_action_controller_spawner = RegisterEventHandler(
    #     event_handler=OnProcessExit(
    #         target_action=joint_trajectory_controller_spawner,
    ...
    # ==========================================================================
    moveit_config = (
        MoveItConfigsBuilder('myCobot320', package_name='primer_paquete_2026')
        .robot_description(
            file_path=os.path.join(tp_share, 'robot_description', 'mycobot_320_m5_2022', 'mycobot_320_m5_2022.xacro')
        )
        .robot_description_semantic(
            file_path=os.path.join(tp_share, 'config', 'mycobot_320_m5_2022', 'moveit', 'mycobot_320_m5_2022.srdf')
        )
        .robot_description_kinematics(
            file_path=os.path.join(tp_share, 'config', 'mycobot_320_m5_2022', 'moveit', 'kinematics.yaml')
        )
        .trajectory_execution(
            file_path=os.path.join(tp_share, 'config', 'mycobot_320_m5_2022', 'moveit', 'moveit_controllers.yaml')
        )
        .planning_pipelines(
        ...
        .joint_limits(
            file_path=os.path.join(tp_share, 'config', 'mycobot_320_m5_2022', 'moveit', 'joint_limits.yaml')
        )
        .pilz_cartesian_limits(
            file_path=os.path.join(tp_share, 'config', 'mycobot_320_m5_2022', 'moveit', 'pilz_cartesian_limits.yaml')
        )
        .to_moveit_configs()
    )

    ompl_yaml_path = os.path.join(tp_share, 'config', 'mycobot_320_m5_2022', 'moveit', 'ompl_planning.yaml')
    ...
    launch_gazebo = IncludeLaunchDescription(
        ...
        launch_arguments=[(
            'gz_args', [
                ...
                ' --gui-config ',
                os.path.join(tp_share, 'config', 'gazebo.config')
            ]
        )]
    )

    # PLANNING SCENE: los 3 objetos del escritorio como CollisionObject. Poses en
    # el frame del robot (offset +0.553 en Z respecto al mundo de Gazebo) -- deben
    # actualizarse a mano si se mueven los objetos en el .world.
    exec_planning_scene = ExecuteProcess(
        cmd=[
            'ros2', 'topic', 'pub', '--once',
            '/planning_scene',
            'moveit_msgs/msg/PlanningScene',
            '{"is_diff": true, "world": {"collision_objects": ['
            '{"id": "bloque_caible", "header": {"frame_id": "world"}, "operation": 0, '
            '"primitives": [{"type": 3, "dimensions": [0.5, 0.02]}], '
            '"primitive_poses": [{"position": {"x": 0.25, "y": 0.0, "z": 0.25}, "orientation": {"w": 1.0}}]}, '
            '{"id": "caja_verde", "header": {"frame_id": "world"}, "operation": 0, '
            '"primitives": [{"type": 1, "dimensions": [0.22, 0.20, 0.10]}], '
            '"primitive_poses": [{"position": {"x": 0.19, "y": 0.20, "z": 0.10}, "orientation": {"w": 1.0}}]}, '
            '{"id": "esfera_amarilla", "header": {"frame_id": "world"}, "operation": 0, '
            '"primitives": [{"type": 2, "dimensions": [0.03]}], '
            '"primitive_poses": [{"position": {"x": 0.19, "y": -0.20, "z": 0.03}, "orientation": {"w": 1.0}}]}'
            ']}, "object_colors": ['
            '{"id": "bloque_caible", "color": {"r": 1.0, "g": 0.0, "b": 0.0, "a": 1.0}}, '
            '{"id": "caja_verde", "color": {"r": 0.0, "g": 1.0, "b": 0.0, "a": 1.0}}, '
            '{"id": "esfera_amarilla", "color": {"r": 1.0, "g": 1.0, "b": 0.0, "a": 1.0}}'
            ']}'
        ],
        output='screen'
    )
    ...
    node_rviz = Node(
        ...
        name='rviz2',
        output='log',
        arguments=['-d', os.path.join(tp_share, 'config', 'display.rviz')],
        parameters=[
    ...
    node_plotjuggler = Node(
        ...
        name='plotjuggler',
        output='log',
        arguments=['-l', os.path.join(tp_share, 'config', 'plotjuggler_layout.xml'),'--ros-args', '--log-level', 'WARN'],
    )
    ...
    return LaunchDescription([
        ...
        event_publish_home_trajectory,
        # event_launch_gripper_action_controller_spawner,  # ver nota más arriba
